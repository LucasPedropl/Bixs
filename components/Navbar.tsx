		...
		<>
			...
			>
				<div className="container mx-auto px-4 sm:px-6 lg:px-8">
					<div className="flex justify-between items-center">
						...
						>
							<div
								className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xl shadow-lg transition-all duration-300 ${useDarkTheme ? 'bg-primary-600 text-white' : 'bg-white text-primary-600'}`}
							>
								B
							</div>
							<span
								className={`text-2xl font-bold tracking-tight transition-colors duration-300 ${useDarkTheme ? 'text-slate-900' : 'text-white'}`}
							>
								BIXS
							</span>
						</Link>
						...
						<div className="hidden md:flex items-center space-x-1">
							<div
								className={`flex space-x-1 mr-6 px-4 py-2 rounded-full ${useDarkTheme ? 'bg-slate-100/50' : 'bg-white/10 backdrop-blur-sm border border-white/10'}`}
							>
								{navLinks.map((link) => (
									<button
										key={link.name}
			...
			>
				...
				<div className="flex-1 px-4 py-8 overflow-y-auto relative z-10">
					<p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-6 pl-2">
						Navegação
					</p>
