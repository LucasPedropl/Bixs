...
const Contracts: React.FC = () => {
	...
	const [activeCamera, setActiveCamera] = useState<'doc' | 'face' | null>(
		null,
	);
	...
	const handleValueChange = (name: string, value: string | boolean) => {
		...
		setErrors((prev) => {
			...
			if (name === 'contratante' && typeof value === 'string') {
				if (!value)
					newErrors.contratante =
						'Nome da contratante é obrigatório.';
			...
			if (name === 'dataInicio' || name === 'dataFim') {
				const isEvent = formData.segmento
					?.toLowerCase()
					.includes('evento');
				if (isEvent) {
					const dataInicio =
					...
					if (dataFim) {
						...
						if (normalizedEndDate < today) {
							newErrors.dataFim =
								'A data de fim não pode ser anterior a hoje.';
						} else if (dataInicio) {
							const [sy, sm, sd] = dataInicio
								.split('-')
								.map(Number);
	...
	const validateForm = () => {
		...
		if (!formData.contratante) {
			...
			newErrors.contratante =
				'Informe o nome completo ou razão social completa.';
		}
		...
		if (!formData.contato) {
			...
			newErrors.contato =
				'Número de telefone inválido. Informe um DDD válido e o número correto.';
		}
		...
		if (isEvent) {
			...
			if (!formData.dataFim) {
				...
				const [endYear, endMonth, endDay] = formData.dataFim
					.split('-')
					.map(Number);
				...
				if (formData.dataInicio) {
					const [startYear, startMonth, startDay] =
						formData.dataInicio.split('-').map(Number);
					const normalizedStartDate = new Date(
						startYear,
						startMonth - 1,
						startDay,
			...
			if (!formData.qtdeMaquinas)
				newErrors.qtdeMaquinas =
					'Quantidade de máquinas é obrigatória.';
		} else if (formData.segmento) {
			if (!formData.qtdeLicencas)
				newErrors.qtdeLicencas =
					'Quantidade de licenças é obrigatória.';
		}
	...
	const sendToWhatsApp = async (pdfBlob: Blob, data: FormData) => {
		try {
			...
			if (!loginResponse.ok)
				throw new Error('Falha no login da API Bixs');
			...
			const uploadResponse = await fetch(
				'https://dev.bixs.com.br/v1/api/upload/media',
				{
					method: 'POST',
					headers: {
			...
			if (!instancesData || instancesData.length === 0) {
				throw new Error(
					'Nenhuma instância do WhatsApp encontrada. Por favor, conecte primeiro.',
				);
			}
			...
			const devPhoneWithCountry = TEST_PHONE.startsWith('55')
				? TEST_PHONE
				: `55${TEST_PHONE}`;
			...
			if (!messageResponse.ok)
				throw new Error(
					'Falha ao enviar mensagem para a empresa via WhatsApp',
				);
			...
			const clientMessageResponse = await fetch(
				'https://dev.bixs.com.br/v1/api/message/messages/send',
				{
					method: 'POST',
					headers: {
			...
			if (!clientMessageResponse.ok)
				throw new Error(
					'Falha ao enviar mensagem para o cliente via WhatsApp',
				);
			...
			alert(
				'O contrato foi gerado, mas houve um erro ao enviar para o WhatsApp. ' +
	...
	const calculatePricing = () => {
		let baseAdesao = 250;
		let baseMensalidade = 189.9;
		let blockEvent = false;
		let eventBlockMessage = '';

		const isEvent = formData.segmento?.toLowerCase().includes('evento');
		const pdvs = isEvent
			? parseInt(formData.qtdeMaquinas || '1')
			: parseInt(formData.qtdeLicencas || '1');

		if (!isEvent) {
			baseAdesao = 250;
			if (pdvs <= 5) baseMensalidade = 189.9;
			else if (pdvs === 6) baseMensalidade = 215.0;
			else if (pdvs === 7) baseMensalidade = 230.0;
			else if (pdvs === 8) baseMensalidade = 240.0;
			else if (pdvs === 9) baseMensalidade = 248.0;
			else if (pdvs === 10) baseMensalidade = 259.0;
			else baseMensalidade = 259.0 + (pdvs - 10) * 8;
		} else {
			baseAdesao = 150;
			let dif = 0;
			...
			if (formData.dataInicio && formData.dataFim) {
				...
				const start = new Date(
					Date.UTC(parseInt(sy), parseInt(sm) - 1, parseInt(sd)),
				);
				const end = new Date(
					Date.UTC(parseInt(ey), parseInt(em) - 1, parseInt(ed)),
				);
		...
		if (formData.cupomDesconto) {
			...
			if (cupom) {
				if (isEvent && cupom.type !== 'adesao') {
					mensagemCupom =
						'Este cupom não é válido para o segmento de Eventos (apenas cupons de adesão são aceitos).';
				} else {
					cupomValido = true;
					if (cupom.type === 'adesao' && cupom.discount) {
						...
						economia =
							baseAdesao -
							(cupom.adesao || 0) +
							(baseMensalidade - (cupom.mensalidade || 0));
								...
								<div className="p-4 mb-8 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
									...
									<div>
										...
										<p className="text-red-700 text-sm mt-1">
											{pricing.eventBlockMessage}
										</p>
										{pricing.eventBlockMessage.includes(
											'WhatsApp',
										) && (
											<a
									...
									<div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-6">
										<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
											...
											<div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
												...
												<div className="flex items-baseline gap-2">
													<p className="text-xl font-extrabold text-primary-600">
														R${' '}
														{pricing.finalMensalidade
															.toFixed(2)
															.replace('.', ',')}
													</p>
													{pricing.cupomValido &&
														pricing.finalMensalidade !==
															pricing.baseMensalidade && (
															<p className="text-sm text-slate-400 line-through">
																R${' '}
																{pricing.baseMensalidade
																	.toFixed(2)
										...
										<div className="pt-4 border-t border-slate-200">
											...
											{formData.cupomDesconto && (
												<p
													className={`text-sm font-medium mt-2 p-3 rounded-lg ${pricing.cupomValido ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'}`}
												>
									...
									<div className="flex flex-col md:flex-row md:items-center gap-3">
										{isDevEnvironment && (
											<button
												...
												className="w-full md:w-auto px-6 py-4 rounded-xl bg-amber-100 text-amber-800 font-bold hover:bg-amber-200 transition-colors"
											>
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
					<div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-in zoom-in-95 fade-in duration-200">
						...
						<p className="text-slate-600 text-center mb-8">
							Nossa equipe recebeu seu contrato e em breve entrará
							em contato com você.
						</p>
